...
class Detector:
    ...
    def detect(self, x_in, thresh, anchors):
        out13, out26, cls = self.net(x_in.to(self.device))
    ...
    def boxFilter(self, output, thresh):
        ...
        output = output.reshape(output.size()[0], output.size()[1],
                                output.size()[2], 3, -1)  # N, H, W, 3, 5 (iou, cx, cy, w, h)
    ...
    def boxReturn(self, idxs, vectors, scaleRate, anchors):
        ...
        batch, idx_cy, idx_cx, boxType = idxs[:, 0], idxs[:, 1], idxs[:, 2], idxs[:, 3]  # [N, H, W, 3(Box Type), 5]
